    ...
    if os.path.exists(FILE_24) and os.path.exists(FILE_25):
        ...
        with st.sidebar:
            ...
            if df_long is not None and not df_long.empty:
                ...
                selected_grades = st.multiselect("Select AY 24-25 Grade (Cohort Tracking)", options=grades, default=grades, key="grade_long",
        ...
        if df_long is not None and not df_long.empty:
            if filtered_df_long.empty:
                ...
                RISE_COLORS_LONG = {"Reviving": "#f27c48", "Initiating": "#0094c9", "Shaping": "#00964d", "Evolving": "#ed1c2d"}

                # ── Retained-cohort logic (first 3 tabs only) ──────────────────
                ...
                # ── TAB 1: MIGRATION (needs retained cohort) ───────────────────
                with mig_tab:
                    st.markdown("### 🧱 Structural Tier Migration (Retained Cohort)")
                    if no_overlap:
                        st.warning("⚠️ No overlapping Student IDs were found between AY 24-25 and AY 25-26 for the current filters. This tab requires matched students across both years. Use the **Subject Wise**, **Geographical Wise**, or **Centre Deep Dive** tabs for year-on-year comparison without requiring matched IDs.")
                    else:
                ...
                # ── TAB 2: SUBJECT EFFICACY (needs retained cohort) ────────────
                with sub_tab:
                    st.markdown("### 📈 YoY Subject Trajectory (Slopegraph)")
                    if no_overlap:
                        st.warning("⚠️ No overlapping Student IDs were found between AY 24-25 and AY 25-26. This slopegraph requires matched students. Use the **Subject Wise** tab for a full subject comparison across all timepoints without requiring matched IDs.")
                    else:
                ...
                # ── TAB 3: GENDER EQUITY (needs retained cohort) ───────────────
                with gen_tab:
                    st.markdown("### 🚻 Gender Equity Tracking")
                    if no_overlap:
                        st.warning("⚠️ No overlapping Student IDs were found between AY 24-25 and AY 25-26. Gender equity tracking requires matched students across both years.")
                    else:
                ...
                # ── TAB 4: SUBJECT WISE — no overlap required ──────────────────
                with subj_tab_long:
                    ...
                    if 'Subject' in filtered_df_long.columns:
                        ...
                        with col_sw1:
                            ...
                            subj_avg = df_subj_view.groupby(['Timepoint', 'Subject'], observed=True)['Obtained Marks'].mean().reset_index()
                        ...
                        with col_sw2:
                            ...
                            if 'Category' in df_subj_view.columns:
                                subj_cat = df_subj_view.groupby(['Subject', 'Timepoint', 'Category'], observed=True).size().reset_index(name='Count')
                ...
                # ── TAB 5: GEOGRAPHICAL WISE — no overlap required ─────────────
                with geo_tab_long:
                    st.markdown("### 🗺️ Geographical YoY Comparison")
                    if 'State' in filtered_df_long.columns:
                        df_geo = filtered_df_long.copy()
                        df_geo['Timepoint'] = pd.Categorical(df_geo['Timepoint'], categories=TIME_ORDER, ordered=True)

                        col_g1, col_g2 = st.columns(2)
                        with col_g1:
                            st.markdown("#### State-wise Average Score (All Timepoints)")
                            state_avg = df_geo.groupby(['State', 'Timepoint'], observed=True)['Obtained Marks'].mean().reset_index()
                        ...
                        with col_g2:
                            ...
                            if 'Category' in df_geo.columns:
                                tp_options_geo = [t for t in TIME_ORDER if t in df_geo['Timepoint'].values]
                                selected_tp_geo = st.selectbox("Select Timepoint", tp_options_geo, index=len(tp_options_geo)-1, key="tp_geo_long")
                                state_cat_filtered = df_geo[df_geo['Timepoint'] == selected_tp_geo]
                                state_cat_grp = state_cat_filtered.groupby(['State', 'Category']).size().reset_index(name='Count')
                        ...
                        el24_state = df_geo[df_geo['Timepoint'] == 'AY24-25 Endline'].groupby('State')['Obtained Marks'].mean()
                        el25_state = df_geo[df_geo['Timepoint'] == 'AY25-26 Endline'].groupby('State')['Obtained Marks'].mean()
                ...
                # ── TAB 6: CENTRE DEEP DIVE — no overlap required ──────────────
                with centre_tab_long:
                    st.markdown("### 🏫 Centre Deep Dive — Individual Centre Comparison")
                    if 'Centre Name' in filtered_df_long.columns:
                        df_cdd = filtered_df_long.copy()
                        df_cdd['Timepoint'] = pd.Categorical(df_cdd['Timepoint'], categories=TIME_ORDER, ordered=True)
                        ...
                        if not df_centre_deep.empty:
                            ...
                            with col_c1:
                                ...
                                centre_avg = df_centre_deep.groupby(['Timepoint', 'Subject'], observed=True)['Obtained Marks'].mean().reset_index()
                            ...
                            with col_c2:
                                st.markdown("#### R.I.S.E Distribution Across Timepoints")
                                if 'Category' in df_centre_deep.columns:
                                    centre_cat = df_centre_deep.groupby(['Timepoint', 'Category'], observed=True).size().reset_index(name='Count')
                            ...
                            el24_all = df_cdd[df_cdd['Timepoint'] == 'AY24-25 Endline']['Obtained Marks'].mean()
                            el25_all = df_cdd[df_cdd['Timepoint'] == 'AY25-26 Endline']['Obtained Marks'].mean()
                            ...
                            if 'Gender' in df_centre_deep.columns:
                                ...
                                if not gen_centre.empty:
                                    gen_avg_c = gen_centre.groupby(['Timepoint', 'Gender'], observed=True)['Obtained Marks'].mean().reset_index()
            ...
            with tab3:
                ...
                if not filtered_df.empty:
                    ...
                    fig_state = px.bar(state_cat, x="Period", y="Percentage", color="Category", facet_col="State Abbr",
                                       ...
                                       category_orders={"Category": ["Reviving", "Initiating", "Shaping", "Evolving"], "Period": ["B", "E"]})
                ...
                if not filtered_df.empty:
                    ...
                    for cat in ["Reviving", "Initiating", "Shaping", "Evolving"]:
                        if cat not in center_piv.columns:
                            center_piv[cat] = 0
                    center_piv_sorted = center_piv.sort_values(by=['Evolving', 'Shaping', 'Initiating', 'Reviving'],
                                                               ascending=[False, False, False, False]).head(10)
                    center_piv_sorted = center_piv_sorted.iloc[::-1]
                    top_centres_long = center_piv_sorted.reset_index().melt(id_vars='Centre Name',
                        value_vars=["Reviving", "Initiating", "Shaping", "Evolving"], var_name='Category', value_name='Percentage')
                ...
                if not base_df.empty and not end_df.empty and 'Student ID' in df.columns:
                    ...
                    if not rtm_df.empty:
                        ...
                        normalize_rtm = st.checkbox("⚙️ Normalize scores (Z-scores) before analysis", value=False,
                            help="Standardizes scores so the Baseline and Endline both have a mean of 0 and standard deviation of 1.")
                        if normalize_rtm:
                        ...
                        st.caption("**Interpretation:** A **negative slope** (trendline going down) suggests the RTM effect is present.")
                        fig_rtm = px.scatter(rtm_df, x="Obtained Marks_BL", y="Score Delta",
                            trendline="ols", trendline_color_override="red", opacity=0.6,
                            color_discrete_sequence=["#636EFA"],
                            labels={"Obtained Marks_BL": "Baseline Score (Z-Score)" if normalize_rtm else "Baseline Score",
                        ...
                        st.caption("**Interpretation:** Students are grouped into 5 equal-sized bins based on their initial Baseline scores.")
                        try:
                            ...
                            fig_binned = px.bar(binned_stats, x='BL_Quintile_Str', y='Avg_Score_Delta',
                                text=binned_stats['Avg_Score_Delta'].apply(lambda x: f"{x:+.2f}"),
                                color='Avg_Score_Delta', color_continuous_scale=px.colors.diverging.RdYlGn,
                            ...
                            try:
                                ...
                                def fig_to_png(fig):
                                    fig.update_layout(
                                        ...
                                        yaxis=dict(showline=True, linecolor="black", linewidth=1.5, showticklabels=True,
                                                   ticks="outside", tickcolor="black", ticklen=5, showgrid=True,
                                                   gridcolor="lightgrey", gridwidth=1, zeroline=True,
                                                   zerolinecolor="black", zerolinewidth=1, mirror=False),
                                        xaxis=dict(showline=True, linecolor="black", linewidth=1.5, showticklabels=True,
                                                   ticks="outside", tickcolor="black", ticklen=5, showgrid=False, mirror=False),
                                    )
                                ...
                                slide1.placeholders[1].text = f"Donor: {selected_donors}\nGenerated automatically via Streamlit"
                                ...
                                if not end_df.empty and 'Subject' in end_df.columns:
                                    subj_counts = (end_df.drop_duplicates(subset=['Student ID', 'Subject'])['Subject'].value_counts().sort_index())
                                ...
                                for subj in all_subjects:
                                    ...
                                    if not subj_base.empty and 'Grade' in subj_base.columns:
                                        ...
                                        grp_bl['Percentage'] = grp_bl.groupby('Grade')['Count'].transform(lambda x: x / x.sum() * 100)
                                        fig_bl_subj = px.bar(grp_bl, x="Grade", y="Percentage", color="Category",
                                            color_discrete_map=RISE_COLORS,
                                            text=grp_bl['Percentage'].apply(lambda x: f'{x:.1f}%' if x > 5 else ''),
                                            category_orders={"Category": ["Reviving", "Initiating", "Shaping", "Evolving"]})
                                        fig_bl_subj.update_layout(barmode='stack', yaxis_title="% of Students",
                                    ...
                                    if not subj_end.empty and 'Grade' in subj_end.columns:
                                        ...
                                        grp_el['Percentage'] = grp_el.groupby('Grade')['Count'].transform(lambda x: x / x.sum() * 100)
                                        fig_el_subj = px.bar(grp_el, x="Grade", y="Percentage", color="Category",
                                            color_discrete_map=RISE_COLORS,
                                            text=grp_el['Percentage'].apply(lambda x: f'{x:.1f}%' if x > 5 else ''),
                                            category_orders={"Category": ["Reviving", "Initiating", "Shaping", "Evolving"]})
                                        fig_el_subj.update_layout(barmode='stack', yaxis_title="% of Students",
                                            margin=dict(l=0, r=0, t=50), showlegend=True,
                                            legend=dict(orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5, title=""))
