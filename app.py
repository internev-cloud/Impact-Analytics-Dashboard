...
try:
    ...
    st.error(
        "Missing `.streamlit/secrets.toml` file or Streamlit Cloud Secrets. Please ensure your Google Client ID and Secret are configured.")
    st.stop()
...
if not st.session_state["logged_in_email"]:
    ...
    with col2:
        ...
        st.markdown("<h2 style='text-align: center; color: #0094c9;'>Staff Analytics Portal</h2>",
                    unsafe_allow_html=True)
        st.markdown(
        ...
        # Create Login Button
        # Note: redirect_uri MUST match what you put in Google Cloud.
        # When deploying, change this to your live URL, e.g., "https://ev-assessments.streamlit.app"
...
st.markdown("<p style='color: gray; font-size: 1.1em;'>Comprehensive Baseline vs. Endline Performance Assessment</p>",
            unsafe_allow_html=True)
...
def load_and_prep_data(file_path):
    common_cols = ['State', 'Centre Name', 'Donor', 'Subject', 'Grade', 'Student ID', 'Gender', 'Total Marks',
                   'Obtained Marks', 'Category', 'Academic Year']
    dfs_to_concat = []
...
if os.path.exists(DATA_FILE):
    ...
    if not df.empty:
        with st.sidebar:
            ...
            if 'Gender' in df_grade_filtered.columns:
                valid_genders = df_grade_filtered[
                    ~df_grade_filtered['Gender'].str.lower().isin(['nan', 'none', 'null', ''])].copy()
        ...
        if filtered_df.empty:
            ...
            with tab1:
                ...
                if not base_df.empty and not end_df.empty and 'Student ID' in df.columns:
                    matched_students = len(pd.merge(base_df[['Student ID', 'Subject']].dropna(),
                ...
                if avg_base is not None and avg_end is not None:
                    ...
                    if sd_base is not None and sd_end is not None:
                        kpi4.metric("Endline Score SD", f"{sd_end:.2f}", delta=f"{sd_end - sd_base:.2f}",
                                    delta_color="inverse")
                    else:
                        kpi4.metric("Endline Score SD", "N/A")

                    # Calculate % of students in "Evolving" (top category)
                    base_evolve = len(base_df[base_df['Category'] == 'Evolving']) / len(base_df) * 100 if len(
                        base_df) > 0 else 0
                    end_evolve = len(end_df[end_df['Category'] == 'Evolving']) / len(end_df) * 100 if len(
                        end_df) > 0 else 0
                    kpi5.metric("Students in 'Evolving'", f"{end_evolve:.1f}%",
                                delta=f"{end_evolve - base_evolve:.1f}%")
                ...
                st.info(
                    "**💡 Understanding Standard Deviation (SD):** SD measures the spread of student scores. A **decrease** (green delta) in SD means scores are becoming more clustered and consistent, indicating that the learning gap between high and low performers is closing. An **increase** (red delta) means the gap is widening.")
                ...
                with col_b:
                    ...
                    fig_rise = px.bar(cat_counts, x="Category", y="Percentage", color="Academic Year",
                                      text=cat_counts['Percentage'].apply(
                ...
                with sub_col1:
                    ...
                    if not base_stacked.empty:
                        fig_base_grade = px.bar(base_stacked, x="Grade", y="Percentage", color="Category",
                                                ...
                                                text=base_stacked['Percentage'].apply(
                                                    lambda x: f'{x:.1f}%' if x > 5 else ''),
                                                category_orders={
                                                    "Category": ["Reviving", "Initiating", "Shaping", "Evolving"]})
                        fig_base_grade.update_layout(barmode='stack', yaxis_title="% of Students",
                ...
                with sub_col2:
                    ...
                    if not end_stacked.empty:
                        fig_end_grade = px.bar(end_stacked, x="Grade", y="Percentage", color="Category",
                                               color_discrete_map=RISE_COLORS,
                                               text=end_stacked['Percentage'].apply(
                                                   lambda x: f'{x:.1f}%' if x > 5 else ''),
                                               category_orders={
                                                   "Category": ["Reviving", "Initiating", "Shaping", "Evolving"]})
                        fig_end_grade.update_layout(barmode='stack', yaxis_title="% of Students",
                                                    margin=dict(l=0, r=0, t=30),
                                                    legend=dict(orientation="h", yanchor="top", y=-0.15,
                                                                xanchor="center", x=0.5, title=""))
                ...
                if not base_stacked.empty and not end_stacked.empty:
                    try:
                        ...
                        if len(common_grades) > 0:
                            ...
                            if best_evo_val > 0:
                                st.success(
                                    f"📈 **Top Excellence Growth:** Grade **{best_evo_grade}** saw the highest shift into the 'Evolving' category, increasing its top-tier share by **{best_evo_val:+.1f}** percentage points from Baseline to Endline.")
                            else:
                                st.warning(
                                    "⚠️ **Excellence Alert:** No grade saw an increase in the 'Evolving' category percentage.")

                            if best_rev_val < 0:
                                st.success(
                                    f"📉 **Highest Risk Reduction:** Grade **{best_rev_grade}** had the most successful intervention for struggling students, reducing its 'Reviving' (lowest tier) population by **{abs(best_rev_val):.1f}** percentage points.")
                            else:
                                st.warning(
                                    "⚠️ **Risk Alert:** No grade successfully reduced their share of students in the 'Reviving' category.")
                        else:
                            st.info(
                                "Insufficient overlapping grades between Baseline and Endline to generate comparative insights.")
                    except Exception as e:
            ...
            with tab3:
                ...
                if not filtered_df.empty:
                    state_cat = filtered_df.groupby(['State', 'Academic Year', 'Category']).size().reset_index(
                        name='Count')
                    ...
                    fig_state = px.bar(state_cat, x="Period", y="Percentage", color="Category", facet_col="State Abbr",
                                       hover_data={"State": True, "State Abbr": False, "Period": False,
                                                   "Academic Year": True},
                                       color_discrete_map=RISE_COLORS,
                                       text=state_cat['Percentage'].apply(
                                           lambda x: f'{x:.1f}%' if not pd.isna(x) and x > 5 else ''),
                                       category_orders={"Category": ["Reviving", "Initiating", "Shaping", "Evolving"],
                                                        "Period": ["B", "E"]})
                ...
                if not filtered_df.empty:
                    ...
                    center_piv = center_cat.pivot(index='Centre Name', columns='Category', values='Percentage').fillna(
                        0)
                    ...
                    fig_top_centres = px.bar(top_centres_long, x="Percentage", y="Centre Name", color="Category",
                                             ...
                                             text=top_centres_long['Percentage'].apply(
                                                 lambda x: f'{x:.1f}%' if x > 5 else ''),
                                             category_orders={
                                                 "Category": ["Reviving", "Initiating", "Shaping", "Evolving"]})

                    fig_top_centres.update_layout(barmode='stack', xaxis_title="% of Students", yaxis_title="",
                                                  margin=dict(l=0, r=0, t=30),
            ...
            with tab4:
                ...
                if not base_df.empty and not end_df.empty and 'Student ID' in df.columns:
                    base_clean = base_df[['Student ID', 'Subject', 'Obtained Marks', 'Category']].dropna(
                        subset=['Student ID'])
                    end_clean = end_df[['Student ID', 'Subject', 'Obtained Marks', 'Category']].dropna(
                        subset=['Student ID'])
                    ...
                    if not paired_df.empty:
                        ...
                        st.caption(
                            "Read rows left-to-right to see student mobility. **Background colors represent transition status:** <span style='color:#82E0AA; font-weight:bold;'>Green (Upward Transition)</span>, <span style='color:#A9A9A9; font-weight:bold;'>Grey (No Transition)</span>, and <span style='color:#FF7F7F; font-weight:bold;'>Red (Downward Transition)</span>.",
                            unsafe_allow_html=True)

                        transition = pd.crosstab(paired_df['Category_BL'], paired_df['Category_EL'],
                                                 normalize='index') * 100
                        ...
                        st.warning(
                            "⚠️ Could not find matching 'Student ID' and 'Subject' between the Baseline and Endline datasets.")
                else:
                    st.info(
                        "⚠️ Both Baseline and Endline datasets with a valid 'Student ID' column are required for this analysis.")
                ...
                if 'Gender' in filtered_df.columns:
                    ...
                    if not gdf.empty:
                        ...
                        for i, g in enumerate(genders_present):
                            with cols[i]:
                                b_mean = g_base[g_base['Gender'] == g][
                                    'Obtained Marks'].mean() if not g_base.empty else None
                                e_mean = g_end[g_end['Gender'] == g][
                                    'Obtained Marks'].mean() if not g_end.empty else None
                        ...
                        with gen_col1:
                            ...
                            fig_gen_avg = px.bar(avg_gen, x="Gender", y="Obtained Marks", color="Academic Year",
                                                 barmode="group",
                        ...
                        with gen_col2:
                            ...
                            gen_cat = gdf.groupby(['Gender', 'Academic Year', 'Category']).size().reset_index(
                                name='Count')
                            gen_cat['Percentage'] = gen_cat.groupby(['Gender', 'Academic Year'])['Count'].transform(
                                lambda x: x / x.sum() * 100)

                            fig_gen_rise = px.bar(gen_cat, x="Academic Year", y="Percentage", color="Category",
                                                  facet_col="Gender",
                                                  color_discrete_map=RISE_COLORS,
                                                  text=gen_cat['Percentage'].apply(
                                                      lambda x: f'{x:.1f}%' if not pd.isna(x) and x > 5 else ''),
                                                  category_orders={
                                                      "Category": ["Reviving", "Initiating", "Shaping", "Evolving"],
                                                      "Academic Year": ["Baseline", "Endline"]})
                            fig_gen_rise.update_layout(barmode='stack', yaxis_title="% of Students",
                    ...
                    st.warning(
                        "⚠️ 'Gender' column is missing from the uploaded dataset. Please ensure your files include a column labeled 'Gender' with values like 'Boy' or 'Girl'.")
            ...
            with tab6:
                ...
                if not base_df.empty and not end_df.empty and 'Student ID' in df.columns:
                    ...
                    base_rtm = base_df[['Student ID', 'Subject', 'Obtained Marks']].dropna(
                        subset=['Student ID', 'Obtained Marks'])
                    end_rtm = end_df[['Student ID', 'Subject', 'Obtained Marks']].dropna(
                        subset=['Student ID', 'Obtained Marks'])
                    ...
                    if not rtm_df.empty:
                        ...
                        normalize_rtm = st.checkbox("⚙️ Normalize scores (Z-scores) before analysis", value=False,
                                                    help="Standardizes scores so the Baseline and Endline both have a mean of 0 and standard deviation of 1. This prevents differences in test difficulty or variance from skewing the RTM analysis.")

                        if normalize_rtm:
                            # Convert raw scores to Z-scores
                            rtm_df['Obtained Marks_BL'] = (rtm_df['Obtained Marks_BL'] - rtm_df[
                                'Obtained Marks_BL'].mean()) / rtm_df['Obtained Marks_BL'].std()
                            rtm_df['Obtained Marks_EL'] = (rtm_df['Obtained Marks_EL'] - rtm_df[
                                'Obtained Marks_EL'].mean()) / rtm_df['Obtained Marks_EL'].std()
                        ...
                        intercept = rtm_df['Score Delta'].mean() - (
                                    slope * rtm_df['Obtained Marks_BL'].mean()) if not pd.isna(slope) else 0.0
                        ...
                        if slope <= -0.1:
                            st.warning(
                                "💡 **Important Insight:** Part of the observed improvement may be due to statistical regression to the mean rather than pure intervention impact. Students who scored exceptionally low on the baseline naturally tend to score closer to the average on the endline.")
                        else:
                            st.success(
                                "💡 **Important Insight:** Observed improvements are less likely driven by RTM. The growth seen across the cohort is more likely attributable to the actual impact of the educational intervention.")
                        ...
                        st.caption(
                            "**Interpretation:** A **negative slope** (trendline going down) suggests the RTM effect is present — meaning students with lower baseline scores tended to improve more, while those with higher baseline scores improved less or dropped.")
                        ...
                        try:
                            ...
                            st.info(
                                "Not enough variance in Baseline scores to generate quintile bins for this selection.")
                        ...
                        st.caption(
                            "Quantifying the strength of the Regression to the Mean effect using linear regression.")
                        ...
                        if slope < -0.3:
                            st.success(
                                f"✔️ **Strong RTM Effect Confirmed:** The strongly negative slope and equation {equation_str} prove that students with lower baselines experienced significantly higher growth.")
                        elif slope < -0.1:
                            st.info(
                                f"ℹ️ **Moderate RTM Effect:** The negative slope and equation {equation_str} indicate a moderate Regression to the Mean pattern.")
                        elif slope < 0:
                            st.warning(f"⚠️ **Weak RTM Effect:** The slope is very close to zero. {equation_str}")
                        else:
                            st.error(
                                f"❌ **No RTM Effect Detected:** The slope is positive ({slope:.2f}), meaning higher-scoring students actually improved *more* than lower-scoring students.")
    ...
    st.error(
        f"⚠️ Data file not found! Please ensure `{DATA_FILE}` is placed in the same folder as this script to populate the dashboard.")
